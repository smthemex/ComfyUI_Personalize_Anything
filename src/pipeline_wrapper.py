...
class RFInversionParallelFluxPipeline(
    DiffusionPipeline,
    FluxLoraLoaderMixin,
    FromSingleFileMixin,
    TextualInversionLoaderMixin,
    ...
    def encode_image(
        ...
        x0 = self.vae.encode(image.to("cuda:0")).latent_dist.sample() #TO DO
        #x0 = self.vae.encode(image.to(self.vae.device)).latent_dist.sample()
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #torch.Size([2, 1024, 64]) torch.Size([2, 768]) torch.Size([2, 512, 4096]) torch.Size([512, 3]) torch.Size([1024, 3])
                print(latents.shape,pooled_prompt_embeds.shape,prompt_embeds.shape,text_ids.shape,latent_image_ids.shape) #torch.Size([2, 1024, 64]) torch.Size([2, 768]) torch.Size([2, 256, 4096]) torch.Size([512, 3]) torch.Size([1024, 3])
    ...
    def invert(
        ...
        Args:
            ...
            timesteps (`List[int]`, *optional*):
                ...
                passed will be used. Must be in descending order.
        """
        dtype = dtype or prompt_embeds.dtype # fix wrapper dtype
        batch_size = 1
        self._joint_attention_kwargs = joint_attention_kwargs
        num_channels_latents = self.transformer.config.in_channels // 4
